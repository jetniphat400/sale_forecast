...
**Update, 2026-09-25 (task 2a, same day): items 1, 2, 3, 8 DONE; item 7 PARTIALLY done (the
...
1. **DONE (task 2a, commit `ca2db9d`).** Delete `forecast/index.html` — stale, orphaned duplicate
   of the live dashboard (factsheet Part 5 item 1; also this task's STATUS.md §4 Locked Decision,
   "`forecast/index.html` is an orphaned stale duplicate..."). Re-confirmed by a fresh repo-wide
   search before deletion: nothing linked to it.
2. **DONE (task 2a, commit `168e5e3`).** `forecast/sales_report.html`'s "MAE / MASE / Bias ต่อฝ่าย"
   table (`#div-results-table`) did not disclose it is Type-level, Combination-model-only, not
   item-level and not "Top-down" (factsheet Part 5 item 2). **Fixed**: a genuine per-division,
   item-level, rolling-origin Top-down figure set exists (`output/summary/
   phaseC_step2_transferability_per_division.csv`, `approach=='Top-down'` rows, generated by
   `src/transferability_all_divisions.py`) — now the PRIMARY table on the page; the old Type-level
   Combination-only figures are kept as a SECONDARY table, labelled exactly for what they are.
   Independently re-verified by the Validator: the page's embedded PEM107 row matches the CSV
   exactly to full floating-point precision (MAE 11.571453087242183, RMSE 14.14489231409701,
   Bias 1.6852751985457102, MASE 0.9940402816237874, n_scored 779) — **level V2**.
3. **DONE (task 2a, commit `168e5e3`).** `forecast/sales_report.html`'s displayed "Usable range"
   end date (2026-08-28) was stale against its own underlying data pulls and the page's own last
   build (factsheet Part 5 item 3). **Fixed**: now derived at build time from the last month
   actually present in `output/data/processed_full_category_sales_monthly_forecastDate.csv`;
...
7. **PARTIALLY DONE.** The `index.html` half is **DONE (task 2a, commits `8650cac`/`9109f59`)**:
   ...
   fresh `Cube_Inventory_Exact` query (PEM101 141,323/5,671/20; PEM103 43/0/0; PEM107 720/70/0;
   CI101/PEM102/PEM104 reported "not assessed," no configured sellable list — **level V2**). The
   `forecast/inventory.html` half (its `on_hand_sellable` input still doesn't separate staging
   stock) **remains OPEN, deferred to task 2b**.
8. **DONE (task 2a, commit `9109f59`).** `index.html`'s stock panel showed only a bare snapshot
   date with no staleness indicator (factsheet Part 5 item 8). **Fixed**: `data_pulled_at`/
   `page_built_at` now shown per METRICS.md §26, with a visible staleness notice where applicable
   (not currently triggered — panel's data is <7 days old post-refresh).
...
- **CONFIRMED, task 2a (commit `168e5e3`).** The on-time delivery chart's `pct_on_time`
  (`output/summary/delivery_by_year.csv`) IS `on_time_exact` — confirmed with file+line:
  `src/investigations/delivery_performance.py`'s `classify_delay()` (lines 71-75) returns
  `"on_time"` only when `days==0` exactly; row-weighted, PEM101 128-item scope, vs. `PlanDelDate`.
  **Fixed**: chart retitled to state this explicitly; `not_late` added for the same years
  (unit-weighted by `ActualQty`, computed from `Cube_CES` `ActualDelDate` vs. `ForecastDelDate`):
  2023=55.4%, 2024=97.3%, 2025=98.9%, 2026=98.3%. Independently re-verified by the Validator: 2023
  and 2024 match the page exactly; 2025 and 2026 show a small, both-additive discrepancy (+4
  rows/+28 units in 2025; +933 rows/+124,385 units in 2026 — Validator's query ran later the same
  day than the page's own pull, consistent with ordinary live-data accrual, not a computation bug
  — **level V1**, inference, not confirmed by a row-level diff). See DATA_MAP.md for the dated
  entry.
- **CONFIRMED AND RESOLVED, task 2a (commit `8650cac`; DATA_MAP.md §4 item 23 updated
  accordingly).** The stock panel's Reserved column DID read `Cube_Backlog`. **Fixed**: switched to
  `Cube_CES` rows with `Status='Backlog'`, deduplicated per METRICS.md §14. Reserved total: 44,361
  units/144 codes (old, `Cube_Backlog`) → 42,121 units/142 codes (new, `Cube_CES`), a real,
  material change (-2,240 units, -5.0%; 42 of 445 codes' values changed). Independently re-verified
  by the Validator against a fresh DB pull: **exact match, 42,121.0 = 42,121.0 — level V2** (using
  the project's own pre-existing Reserved-quantity convention, `ActualQty.fillna(0) +
  BacklogQty.fillna(0)` per row, `src/phaseE1fix_part0_backlog_source.py` — see DATA_MAP.md's new
  note on this, since a literal "sum ActualQty alone" reading would have wrongly flagged a
  discrepancy).
- **CONFIRMED, task 2a (commit `9109f59`).** The Phase 1 "Trend Pricelist Omni 2024–2026" tab
  (`#omniTab`) IS still on the older 448-code basis including hidden pricelist sheets (the tab's
  own text still literally reads "448 รหัส"). **No generator script exists anywhere in the repo**
  for this tab's embedded data (thorough repo-wide search, task 2a) — per instruction, no new
  generator was written this task; the tab is instead labelled with its own data date (2026-08-25)
  and states plainly it uses the 448-code basis and is not refreshed by this pipeline. **New,
  well-defined open item**: writing a generator for this tab (to migrate it onto the current
  445-visible-sheet registry, per the PRICELIST RULE) is itself a real, undone piece of work —
  scheduled below.

**New item, added task 2a**: write a generator script for `index.html`'s "Trend Pricelist Omni"
tab, migrating its embedded item scope from the 448-code (incl. hidden sheets) basis to the current
445-visible-sheet registry (`src/pricelist_reader.py`'s convention) — no such script exists today;
the tab remains hand-maintained and stale until one is written.

**"Refresh every page's data... with METRICS.md §26 timestamps" — DONE for task 2a's scope**
(`index.html` both tabs + stock panel, `forecast/sales_report.html`, and `forecast/inventory.html`'s
own §26 timestamps only — commits `3b76d25`, `3693db6`, `8650cac`, `9109f59`, `168e5e3`). Forward-
test log (`output/summary/forward_test_log_all_divisions.csv` — the literal path
`forward_test_log.csv` named in earlier tasks and PROJECT_GRAPH.md's T1 node does not exist on
disk; this is the current all-divisions log per STATUS.md:4531) confirmed byte-identical before and
after the refresh (SHA-256 `3adb765139b576c508a93bf0e21b3562d48711d4d04b002800eb5c55e7576a04`,
