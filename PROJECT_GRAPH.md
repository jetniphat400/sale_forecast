...
| Q21 | **in progress — data route, corrected 2026-09-24, was "blocked on a person"** | Q10 (implicitly) | G3 | Proposed this task, Part 3. Value meanings recorded 2026-09-24 (DATA_MAP.md §2, level A, business-confirmed): MTO = made to order, MTS = made to stock, ETO = engineering to order. **Per CONVENTIONS.md's error-review rule 7 ("do not mark a question blocked on a person until the data route is exhausted"): `manufacturing_type` in `cube_Sale_APD` has NOT yet been tested as a per-item classification route (e.g. mode/majority-vote per item) — that test is proposed next, before this question is marked blocked on a person.** A per-item classification is still not verified — the field remains order-level, not fixed per item (100/113 items show more than one value) — the data route is untested, not exhausted. | **data (untested: manufacturing_type mode/majority-vote per item), not yet "person"** |
...
| Q21 | Which items are made-to-stock vs. made-to-order vs. engineer-to-order? | **Partial, and known to be unreliable at face value.** `manufacturing_type` (MTS/MTO/ETO) exists in `cube_Sale_APD` but is an ORDER-level attribute, not a fixed per-item classification — 100 of 113 items show more than one value across their own sales rows (DATA_MAP.md §2, manufacturing_type). Using it directly as a per-item classification would repeat a known trap. Definitions recorded 2026-09-24 (level A, business-confirmed, DATA_MAP.md §2): MTO = made to order, MTS = made to stock, ETO = engineering to order. **Status corrected 2026-09-24 (CONVENTIONS.md error-review rule 7): moved from "blocked on a person" to "in progress — data route." `manufacturing_type`'s own mode/majority-vote per item is an untested data route that must be tried before this is marked blocked on a person — proposed as the next step, not yet run in this task.** | A per-item MTS/MTO/ETO classification would need to be derived (e.g. a mode or business rule, testable directly from `manufacturing_type`, next step) or confirmed by the business if the derived route proves unreliable — not read off the raw column directly either way. |
